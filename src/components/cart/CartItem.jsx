import React, { useState } from 'react';
import cartCSS from './Cart.module.css';
import { connect } from 'react-redux';
import {
  removeFromCart,
  adjustItemQty,
} from '../../redux/shopping/shopping-actions';

function CartItem({ itemData, removeFromCart, adjustItemQty }) {
  const [input, setInput] = useState(itemData.qty);

  const onChangeHandler = (e) => {
    // console.log(e.target.value)
    setInput(e.target.value);
    adjustItemQty(itemData.id, e.target.value);
    // reason why putting in e.target instead of input is that sometimes state takes a while to update so it would be the old state
  };

  return (
    <div className={cartCSS.box}>
      <img
        className={cartCSS.cartimg}
        src={itemData.image}
        alt={itemData.title}
      />
      <div className="d-flex flex-column justify-content-around">
        <h4>
          <b>{itemData.title}</b>
        </h4>
        <p>{itemData.description}</p>
        <p>
          <b>${itemData.price}</b>
        </p>
      </div>
      <div className="d-flex flex-column justify-content-around">
        <div>
          <label for="quantity">Quantity</label>
          <input
            style={{ width: '70px' }}
            className="me-4"
            name="quantity"
            id="quantity"
            type="number"
            placeholder={input}
            onChange={onChangeHandler}
          />
        </div>
        <button onClick={() => removeFromCart(itemData.id)}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
            height="24"
            fill="currentColor"
            class="bi bi-trash3-fill"
            viewBox="0 0 16 16"
          >
            <path d="M11 1.5v1h3.5a.5.5 0 0 1 0 1h-.538l-.853 10.66A2 2 0 0 1 11.115 16h-6.23a2 2 0 0 1-1.994-1.84L2.038 3.5H1.5a.5.5 0 0 1 0-1H5v-1A1.5 1.5 0 0 1 6.5 0h3A1.5 1.5 0 0 1 11 1.5Zm-5 0v1h4v-1a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5ZM4.5 5.029l.5 8.5a.5.5 0 1 0 .998-.06l-.5-8.5a.5.5 0 1 0-.998.06Zm6.53-.528a.5.5 0 0 0-.528.47l-.5 8.5a.5.5 0 0 0 .998.058l.5-8.5a.5.5 0 0 0-.47-.528ZM8 4.5a.5.5 0 0 0-.5.5v8.5a.5.5 0 0 0 1 0V5a.5.5 0 0 0-.5-.5Z" />
          </svg>
        </button>
      </div>
    </div>
  );
}

const mapStateToProps = (dispatch) => {
  return {
    removeFromCart: (id) => dispatch(removeFromCart(id)),
    adjustItemQty: (id, value) => dispatch(adjustItemQty(id, value)),
  };
};

export default connect(null, mapStateToProps)(CartItem);
